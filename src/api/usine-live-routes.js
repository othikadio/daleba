        ...
        for (let i = 0; i < opps.length; i += THROTTLE_CHUNK) {
          ...
          await Promise.all(chunk.map(async (opp) => {
            ...
            try {
              ...
              // ⛔ USINE STOP if (en?.notifyProposal) await safeCall(() => en.notifyProposal(opp, proposalText, { pricing: proposalPricing, paymentUrl: proposalPricing?.paymentUrl || null }), null);
